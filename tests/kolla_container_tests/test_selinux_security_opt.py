...
podman_worker_file = os.path.join(
    ansible_dir, 'module_utils', 'kolla_podman_worker.py'
)
    ...
    for k, v in module.params.items():
        if v is None:
            if k in common_options_defaults:
                if (k not in new_args and
                        common_options_defaults[k]
                        is not None):
...
class TestPodmanWorkerSecurityOpt(base.BaseTestCase):
    ...
    def test_empty_list_security_opt_triggers_default_label(self):
        """Empty list is treated same as None - default label is applied."""
        self.fake_data['params']['name'] = 'rabbitmq'
        self.fake_data['params']['security_opt'] = []
        self.pw = get_PodmanWorker(self.fake_data['params'])
        kwargs = self.pw.prepare_container_args()
...
class TestCompareSecurityOpt(base.BaseTestCase):
    ...
    def test_handles_none_host_config(self):
        """None HostConfig raises AttributeError in compare_security_opt.

        because None.get() is not caught by the existing KeyError/TypeError
        handlers in compare_security_opt.
    ...
    def test_no_change_when_label_matches_default(self):
        """Check Container with SecurityOpt

        matching the generated default: no diff.
        """
...
class TestLabelFormatConsistency(base.BaseTestCase):
    """The generated label format must be identical for Docker and Podman."""
